...
function buildUpcomingCard(consultation) {
  ...
  details.textContent = `${timing.date}${timing.time ? ` @ ${timing.time}` : ""}`;
...
function getAvailableExperts() {
  const experts = expertAuthStore?.readExperts ? expertAuthStore.readExperts() : [];
...
function attachNavigation() {
  ...
  if (consultationModalForm) {
    consultationModalForm.addEventListener("submit", (event) => {
      ...
      const result = consultationStore.createConsultationRequest({
