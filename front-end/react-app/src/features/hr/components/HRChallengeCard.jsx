  ...
  return (
    <article className="hr-challenge-card">
      <div className="hr-challenge-card-title">
        <span className={`hr-challenge-mark ${getTone(challenge.type)}`}>{getSymbol(challenge.type)}</span>
        <div><h3>{challenge.name}</h3><span className="hr-challenge-tag">{challenge.type}</span></div>
      </div>
      <div className="hr-challenge-metrics">
        <div><span>Reward</span><strong>{challenge.reward} points</strong></div>
        <div><span>Deadline</span><strong>{challenge.deadline || "Open"}</strong></div>
...
function getSymbol(type = "") { return type.toLowerCase().includes("fitness") ? "↗" : type.toLowerCase().includes("health") ? "◖" : "★"; }
